...
<!-- 言語モデルが，文字ベースであれば，入出力層の素子数 (ユニット数，あるいは次元とも言う) が比較的少なくて済む。
一方，単語ベースの言語モデルでは，入出力層の素子数 が大きくなる。
Transformer (GPT や BERT など現代的言語モデルはほとんどが Transformer を基本構成要素としている) では，入出力層の次元は 32000 とすることが多い。

入力層が，ワンホット表現で，中間層が埋め込み表現，出力層がワンホット表現

ところが次の要素を予測するときに，ウィンドウを使う -->
