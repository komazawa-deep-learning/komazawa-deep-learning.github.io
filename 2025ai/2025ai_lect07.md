...
* [ROC, AUC 曲線実習](https://colab.research.google.com/github/komazawa-deep-learning/komazawa-deep-learning.github.io/blob/master/2025notebooks/2025_0604ROC_AUC_demo.ipynb){:target="_blank"}
...
機械学習には，教師あり学習と教師なし学習とがある。

この授業では，
* 教師あり学習の例としてサポートベクタマシン，ロジスティック回帰，線形判別分析を取り上げた。
  * さらに，教師あり学習には分類と回帰とが存在する。
  * 分類とは教師信号が離散量であるのに対して，回帰は教師信号が連続量である。
* 教師なし学習の例として主成分分析 と t-SNE を取り上げた。

## 本日のキーワード

* TP: True Positive (ヒット)
* FP: False Positive (虚報)
* TN: True Negative (正しい棄却)
* FN: False Negative (ミス)
* 精度 (accuracy)
* 適合率 (precision)
* 再現率 (recall)
* F1 値 (F1-score)
