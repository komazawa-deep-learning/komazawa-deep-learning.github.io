...
The architecture of the guided search 2.0. Modified from \cite{1994Wolfe_guidedSearch2} Fig. 2
</center>
<!--- **bottom-up** and **top-down** processes-->

<!--# Guided Search 2.0 was the orignal top-down model-->

# 注意の研究史 (8) 計算モデル

- コッホ と ウルマン (1985) 勝者専有 Winner-take-all [WTA (YouTube)](https://youtu.be/iyIOl-s7JTU)
...
(a) 背側および腹側の定位ネットワーク（Corbetta&Shulman(2002) より引用）。背側注意ネットワーク（薄緑）は、前頭眼野（FEF）と頭頂間溝/上頭頂葉（IPS/SPL）から構成される。腹側注意ネットワーク（青緑）は、側頭頭頂接合部（TPJ）と腹側前頭皮質（VFC）の領域から構成される。<br/>
(b) 実行制御系の 2 つのネットワーク。丸で囲まれた領域は、Posner&Petersen(1990) による実行制御系の元の構成要素を示す。残りの領域は、元の帯状蓋蓋系（黒）の精緻化と前頭頭頂系（黄色）の追加によるもの（Dosenbach+(2007) より改変）。<br/>
(c) 別々の制御系を反映した安静時の相関。この図は脳の 3 つの視野（左：背側、中央：傾斜側方、右：正中）を示す。これらの分離可能な安静時ネットワークは、図 a と b に示されている機能基準に基づく区別と一致している。背側注意（緑）、腹側注意（青緑）、帯状蓋蓋部（黒）、前頭頭頂部（黄） (Power+(2011)より改変）。
...
<img src="/2025assets/2015Itti_Fig3.svg">
<img src="/2025assets/2015Itti_Fig4.svg">

<!--
- [最近提案されたモデル Itti ら](./assets/2015Itti_Fig3.svg)
- [上の比較画像](./assets/2015Itti_Fig4.svg)
-->
...
<!-- Buschman (2007)
<center>
<img src="./assets/2007Buschman_FigS1.svg" style="width:64%">
</center>
-->
