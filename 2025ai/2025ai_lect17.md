...
状態空間モデル，カルマンフィルタ，フィルタリング，予測，平準化，<!--https://qiita.com/hanon52_/items/7f03621414c59f06d7ca-->
...
$$\begin{aligned}
\mb{y} &= \mb{Xw}\\
\mb{X}^{\top}\mb{y} &= \mb{X}^{\top}\mb{Xw}\\
(\mb{X}^{\top}\mb{X})^{-1}\mb{y} = (\mb{X}^{\top}\mb{X})^{-1}(\mb{X}^{\top}\mb{X})\mb{w}\\
(\mb{X}^{\top}\mb{X})^{-1}\mb{y} = \mb{w}
\end{aligned}$$
