...
前回 12 月 19 日は突然休講としてしまい，すみませんでした。
お詫びいたします。

* [補講 01月16日 補講 12月19日分 Google Meet <img src="/2025assets/128px-Google_Meet_icon_2020.png" width="02%;"> 1:00–2:30pm https://meet.google.com/zji-ebna-cvm](https://meet.google.com/zji-ebna-cvm){:target="_blank"}
* 上記補講に出席できない方は事前に連絡をください。

* [課題提出用フォルダ](https://drive.google.com/drive/u/6/folders/1gC9egGuI4ewXQbXhIQB6yU4tydq9eP5i){:target="_blank"}

## 実習ファイル

* [Stable-baselines3 を用いた PPO デモ CartPole 1 <img src="/assets/colab_icon.svg">](https://colab.research.google.com/github/komazawa-deep-learning/komazawa-deep-learning.github.io/blob/master/2025notebooks/2025_1121StableBaselines3_1_getting_started.ipynb){:target="_blank"}
...
RLHF, PPO, 視覚-言語-行動 モデル，Agent 57, 短期記憶，エピソード記憶，経験再生，内発的動機づけ

# RLHF

RLHF: Reinforcement Learning with Human Feedbacks: 人間のフィードバックからの強化学習

## (chat)GPT における RLHF

OpenAI の GPT 系列（ChatGPT を含む）は、
1. 大規模事前学習（言語モデリング）
2. 人間の好みを反映する微調整（RLHF）

の二段階（実際は三段階）で作られている。この RLHF の最終最適化で用いられる代表的手法が **代理方針最適化 Proximal Policy Optimization（PPO）** である。
...
## PPO 
* 安定性：方針（言語モデル）の更新が「行き過ぎない」よう制約
* 実装容易性：TRPO より簡単で大規模モデルに適合
* サンプル効率：人手評価が高価でも学習が回る

* PPO は「前のモデルから大きく逸脱しない範囲で、報酬を最大化」するため設計
* PPO の目的関数（代表形）は クリッピング付きサロゲート：

$$
\mathcal{L}^{\text{CLIP}}(\theta) = \mathbb{E}_t\left[\min\left(r_t(\theta)\,\hat A_t,\; \mathrm{clip}(r_t(\theta), 1-\epsilon, 1+\epsilon)\,\hat A_t \right)\right]
$$

* $\displaystyle r_t(\theta)=\frac{\pi_\theta(a_t\vert s_t)}{\pi_{\theta_{\text{old}}}(a_t \vert s_t)}$：新旧方策の比
<!-- * $r_t(\theta)=\frac{\pi_\theta(a_t|s_t)}{\pi_{\theta_{\text{old}}}(a_t | s_t)}$：新旧方策の比 -->
* $\hat{A}_t$：アドバンテージ（「その行動は平均より良かった？」）
* $\epsilon$：更新幅の上限（逸脱防止）

意味：改善方向は尊重するが、比率が閾値を超える更新は切り捨てて暴走を防ぐ。

<!-- まず第一に，大規模ではない，言語モデルについて考えます。言語モデルは，機械翻訳などでも使われる技術です。DeepL や Google 翻訳で，使っている方もいることでしょう。 -->

<!-- chatGPT を使える方は，上記太字のキーワードについて，chatGPT に質問してみることをお勧めします。とりわけ 注意 については，認知，視覚，心理学との関連も深く，注意の障害は，臨床，教育，発達などの分野と関係する
。 -->

### GPT における RLHF + PPO の流れ

1. 事前学習（SFT 前段）
次トークン予測で巨大コーパスから言語能力を獲得。
2. SFT（教師あり微調整）
人間が書いた良回答で初期方針 $\pi_{\theta_{\text{old}}}$ を作る。
3. 報酬モデル（RM）学習
人間の「どちらが良いか」比較から 好み関数 $R_\phi$ を学習。
4. PPO による最適化
...
<!-- 実装では、KL ペナルティ（新旧モデルの乖離）を明示的に加える変種も一般的です。 -->

### なぜ「言語生成モデル」に 強化学習で開発された PPO が用いられたのか？
...
* 報酬ハック：RM の穴を突く挙動（→ 定期的な RM 再学習が必要）
* 多目的最適化：有用性・安全性・誠実性のトレードオフ
* 長期整合：短期報酬に引っ張られやすい（長文一貫性）

#### まとめ

* GPT の「人間らしさ」は RLHF + PPO が中核
* クリッピングで安定更新、KL 制約で逸脱防止
* 大規模言語モデルの実運用に耐える 実践的 RL

<!-- 必要であれば、PPO を言語モデルに実装する擬似コードや、TRPO・DPO との比較、**アドバンテージ推定（GAE）**の詳細も掘り下げます。 -->
...
* 目的：方策改善をしつつ KL が一定以下の「信頼領域」内で更新
* 典型形：

$$
\max_\theta \ \mathbb{E}[r_t(\theta)\hat A_t] \quad \text{s.t.}\quad \mathbb{E}\left[\mathrm{KL}(\pi_{\theta_{\text{old}}}\|\pi_\theta)\right]\le \delta
$$

* ここでの更新は 自然勾配に近い（Fisher の情報行列を使う）ので
* 理論的に “monotonic improvement” に寄せられる
* 反面<!--❌--> 実装が重い（共役勾配、線形探索、2次近似、分散実装が面倒）

**PPO（Proximal Policy Optimization）**
* TRPO の 制約付き最適化を、実装しやすい クリッピングで近似
* "大きく変えすぎる更新" を 目的関数側で切り捨てる：
* 実装が簡単、GPU /分散 で回しやすい
* 経験的に安定・強い
* 駄菓子菓子<!--❌--> TRPO ほどの理論保証は薄い（ただし実用上は十分）

TRPO は KL 制約を厳密に守る、一方 PPO は「守るように作った近似で回す」。

###### 文献情報

* DPO: Rafailov et al., Direct Preference Optimization: Your Language Model is Secretly a Reward Model, NeurIPS 2023 / [arXiv:2305.18290](https://www.arxiv.org/abs/2305.18290){:target="_blank"}.
* KTO: Ethayarajh et al., KTO: Model Alignment as Prospect Theoretic Optimization, [arXiv:2402.01306](https://www.arxiv.org/abs/2402.01306){:target="_blank"}（ICML 2024掲載情報もあり）。
* ORPO: Hong et al., ORPO: Monolithic Preference Optimization without Reference Model, [arXiv:2403.07691](https://www.arxiv.org/abs/2403.07691){:target="_blank"}.
* IPO: Garg et al., Your Language Model is Secretly a Preference Classifier, [arXiv:2502.16182](https://www.arxiv.org/abs/2502.16182){:target="_blank"}.

# 視覚言語行動モデル VLA model
<!--https://en.wikipedia.org/wiki/Vision-language-action_model)<!-- # Vision-language-action model-->

<div class="figcenter">

<iframe width="705" height="397" src="https://website.pi-asset.com/v2/upload/PhysicalIntelligence_2min_v2_out.mp4" title="Physical Intelligence (π)" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe><br/>
source: https://www.pi.website/blog/pi0
</div>

<div class="figcenter">
<img src="https://upload.wikimedia.org/wikipedia/commons/4/4c/General_architecture_of_a_vision-language-action_model.png" width="66%;">
<div class="figcaption">

視覚-言語-行動モデルの一般的な構成。このモデルは、潜在表現で符号化されたテキスト指示と画像観測を入力として受け取る。行動復号化器はこの表現を受け取り、低レベルのロボット行動系列を生成する。
...
ロボット学習において、視覚言語行動モデル（VLA）とは、視覚、言語、行動を統合するマルチモーダル基盤モデルの類である。ロボットの周囲環境を示す入力画像（または動画）とテキスト指示が与えられると、VLA は要求された課題を達成するために実行可能な低レベルのロボット行動を直接出力する[1]。
<!-- In robot learning, a vision-language-action model(VLA) is a class of multi-modal foundation models that integrates vision, language and actions. Given an input image (or video) of the robot's surroundings and a text instruction, a VLA directly outputs low-level robot actions that can be executed to accomplish the requested task[1].-->

VLA は一般的に、視覚言語モデル（VLM、すなわち視覚機能を拡張した大規模言語モデル）を、視覚的観察と言語指示をロボットの軌跡と対応付けた大規模データセット上で微調整することで構築される[2]。これらのモデルは、画像観察と自然言語記述を潜在空間内の分布に変換する視覚言語エンコーダ（視覚トランスフォーマー）と、この表現をロボット上で直接実行可能な連続出力動作に変換する動作デコーダを組み合わせている[3]。
...
この概念は 2023 年 7 月、Google DeepMind が RT-2 によって先駆的に開発した。RT-2 はエンドツーエンド操作課題向けに適応された VLM であり、知覚・推論・制御を統合する能力を持つ[4]。
...
Upper panel: schematic detailing the quantities that define free-energy. These include states of the brain $\mu$ and quantities describing exchange with the environment; sensory input  $y=g(\vartheta,a)+z$ and action $\alpha$ that changes the way the environment is sampled. The environment is described by equations of motion, $\dot{\vartheta}=f(\vartheta,\alpha)+w$ which specify the dynamics of environmental causes $\vartheta$. Brain states and action both change to minimise free-energy, which is a function of sensory input and a probabilistic representation (recognition density) $q(\vartheta,\mu)$ encoded by $\mu$. 
</p>

<p align="left" style="width:79%">
Lower panel: alternative expressions for the free-energy that show what its minimisation entails. For action, free-energy can only be suppressed by increasing the accuracy of sensory data (i.e. selectively sampling data that are predicted by the representation). Conversely, optimising brain states make the representation an approximate conditional density on the causes of sensory input. This optimisation makes the free-energy bound on surprise tighter and enables action to avoid surprising sensory encounters.
</p>
</center>
...
Generative models in the brain: to suppress free-energy one needs a probabilistic generative model of how the sensorium is caused. These models $p(y,\vartheta)=p(y\vert\vartheta)p(\vartheta) entail the likelihood, p(y\vert\vartheta)$ of getting some data, $y$, given their causes $\vartheta\supset\left\{x(t),\theta,\lambda\right}$ and prior beliefs $p(\vartheta)$. The models employed by the brain have to explain a world with complex dynamics on continuous states. Hierarchical dynamic models provide a general form and specify sensory data as a mixture of predictions (based on causes) and random effects:
...
Schematic detailing the neuronal architectures that might encode a density on the states of a hierarchical dynamic model. This shows the speculative cell s of origin of forward driving connections that convey prediction error from a lower area to a higher area and the backward connections that construct pr edictions [11,20]. These predictions try to explain away prediction error in lower levels. In this scheme, the sources of forward and backward connection s are superficial and deep pyramidal cells, respectively. The equations represent a gradient descent on free-energy under the hierarchical dynamic models of Box 2 (see Ref. [19] for details). State-units are in black and error-units in red. Here, neuronal populations are deployed hierarchically within three cortical areas (or macro-columns). Within each area, the cells are shown in relation to cortical layers: supra-granular (SG) granular (L4) and infra-granular (IG) layers. In this figure, subscripts denote derivatives.
</p>
...
The figure shows the dependencies among the quantities that define free energy. These include the internal states of the brain $\mu(t)$ and quantities describing its exchange with the environment: sensory signals (and their motion)

$$
\widetilde{s}(t)=\left[s,s',s'',\ldots\right]^\top
$$

plus action $a(t)$. The environment is described by equations of motion, which specify the trajectory of its hidden states. The causes 
...
Internal brain states and action minimize free energy $F\left(\widetilde{s},\mu\right)$, which is a function of sensory input and a probabilistic representation $q(\vartheta\vert\mu)$ of its causes. This representation is called the recognition density and is encoded by internal states $\mu$.

<center>
<img src="./assets/2010Friston_box1a.svg" style="width:79%">
</center>

The free energy depends on two probability densities: the recognition density $q(\vartheta\vert\mu)$ and one that generates sensory samples and their causes, $p\left(\widetilde{s},\theta\vert m\right)$. The latter represents a probabilistic generative model (denoted by $m$), the form of which is entailed by the agent or brain.  The figure below provides alternative expressions for the free energy to show what its minimization entails: action can reduce free energy only by increasing accuracy (that is, selectively sampling data that are predicted). Conversely, optimizing brain states makes the representation an approximate conditional density on the causes of sensory input. This enables action to avoid surprising sensory encounters. A more formal description is provided below. 
