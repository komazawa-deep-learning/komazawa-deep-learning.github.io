...
この平均偏差ベクトルの意味を考える.
すべての要素が $1$ であるベクトル ${\bf 1}=(\;\overbrace{1,1,\ldots,1}^{n個}\;)'$ によって張られる部分空間 $L\Brc{\mb{1}}$ への射影行列をつくると
...
したがって, 平均偏差ベクトルは, $\mb{y}-\Bar{y}{\bf 1}=\mb{y}-\mb{Py}=\Brc{\mb{I}-\mb{P}}\mb{y}$

すなわち, 平均偏差ベクトルとは $L\Brc{{\bf 1}}$ の補空間への射影ベクトルである。

この平均偏差ベクトルの長さの 2 乗 $\Norm{x}^2$ をデータ数で割ったものは
...
=\frac{\IP{\mb{x}}{\mb{y}}}{\Norm{\mb{x}}\Norm{\mb{y}}}=\cos\theta_{xy}
$$
</div>
...
<img src="/2024assets/2024_0514Pythagoras.svg">
...
線形判別分析は，偉大な統計学者である R.A.フィッシャー卿によって発明され，彼は 1936 年の論文で花の分類に用いることに成功した。
...
このことは Belhumeur, Hespanha, Kriegman も認識しており，[3] で顔認識に判別分析を適用している。
