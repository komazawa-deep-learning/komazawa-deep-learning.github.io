...
* [課題提出用フォルダ](https://drive.google.com/drive/u/6/folders/1FrP_odpFp4MyrK9LTIFw_9ygPZMRs3Re){:target="_blank"}
...
### <!--スライド6　--> 線形モデルの限界：XOR 問題

* 単層では線形分離できない例（XOR）
* 直観：境界が 1 本の直線（平面）しか作れない
...
* ロジスティック回帰 $\ne$ パーセプトロン（確率化/閾値化の違い）
* 多層化＋非線形 → 表現学習
* 学習＝損失を下げる（最適化）／Backprop は責任分配
...
* ニューラルネットワークが **特徴量設計** をどう置き換えるか，を、HOG/SIFT（人手特徴）との対比で 200〜300 字で説明せよ。
