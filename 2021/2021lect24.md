...
<div class="figcaption">
パレイドリアのシミュレーション。左:入力画像，右:幻視の例 (浅川,2019) より
</div>
</div>
</center>
...
* MCMC の代わりに，サンプリングを最適化に置換
* 事後分布を決定論的に近似
* 確立された最適化手法（ブロック座標上昇法、Newton-Raphson、内点法）を使用。
* 大規模な問題に対して MCMC よりも高速でスケーラブル。
* MCMC にはないバイアスがある。
* 完全ベイズおよび経験的ベイズ推論のための有用な枠組み
...
もし潜在変数がなければ，パラメータ推定のためには **最尤推定法** maximum likelihood} が一般に用いられる。
